...
setGlobals () {
	CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/examples/e2e/crypto/peer/peer$1/localMspConfig
...
PEER=0
setGlobals $PEER
peer chaincode query -C myc1 -n mycc -c '{"Args":["query","a"]}' >log.txt 2>&1
res=$?
cat log.txt
...
peer chaincode invoke -C myc1 -n mycc -c '{"Args":["invoke","a","b","10"]}' >log.txt 2>&1
res=$?
cat log.txt
